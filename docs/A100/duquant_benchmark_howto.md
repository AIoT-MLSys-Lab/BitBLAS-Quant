...
### 1. Shape-collapsed benchmark (`tools/pi_duquant_benchmark.py`)
...
```bash
python tools/pi_duquant_benchmark.py \
  --layer-log pi_llm_ditmlp_layers.json \
  --batch 1 \
  --warmup 5 \
  --runs 20 \
  --output reports/pi_duquant_w4a8.md
```
...
### 2. Sequential benchmark (`tools/pi_duquant_sequence_benchmark.py`)
...
```bash
python tools/pi_duquant_sequence_benchmark.py \
  --layer-log pi_llm_ditmlp_layers.json \
  --batch 1 \
  --warmup 2 \
  --runs 8 \
  --output reports/pi_duquant_w4a8_sequence.md
```
